...
distribution/docker
docker build -f Dockerfile.orkaudio.neo -t thanhtung2106/orkaudio .
docker run -it --net=host --restart=always --privileged=true -v /var/log/orkaudio:/var/log/orkaudio  -v /etc/orkaudio:/etc/orkaudio thanhtung2106/orkaudio:latest 
docker run -d --net=host --restart=always --privileged=true --name=orkaudio -v /var/log/orkaudio:/var/log/orkaudio  -v /etc/orkaudio:/etc/orkaudio thanhtung2106/orkaudio:latest
# after run it in first time, just stop container, copy config.xml logging.properties to /etc/orkaudio and start it again
```
